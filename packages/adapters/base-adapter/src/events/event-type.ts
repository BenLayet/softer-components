...
import type { SofterEvent, SofterEventSource } from "./softer-event";
...
export function toEventTypeString(event: SofterEvent, prefix: string): string {
  ...
  parts[SOURCE_INDEX] = event.source;
...
export function parseEventTypeString(
  ...
  const source = parts[SOURCE_INDEX] as SofterEventSource | undefined;
