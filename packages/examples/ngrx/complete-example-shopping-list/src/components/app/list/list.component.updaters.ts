...
export const stateUpdaters: StateUpdaters<Contract, State> = {
  ...
  nextItemNameChanged: ({ state, payload: nextItemName }) => {
    assertIsNotUndefined(state, 'State should be defined when handling nextItemNameChanged event');
  ...
  saveRequested: ({ state }) => {
    assertIsNotUndefined(state, 'State should be defined when handling saveRequested event');
    state.isSaving = true;
    state.errors = {};
  },
  saveFailed: ({ state, payload: errorMessage }) => {
    assertIsNotUndefined(state, 'State should be defined when handling saveFailed event');
    state.errors['SAVE_FAILED'] = errorMessage;
    state.isSaving = false;
  },
  saveSucceeded: ({ state }) => {
    assertIsNotUndefined(state, 'State should be defined when handling saveSucceeded event');
    state.isSaving = false;
    state.errors = {};
  },
};
