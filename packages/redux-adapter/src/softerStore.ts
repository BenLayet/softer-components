import { configureStore, createListenerMiddleware, createReducer, ListenerMiddlewareInstance, ThunkDispatch, UnknownAction } from '@reduxjs/toolkit';
import {
    ComponentDef,
...
export function configureSofterStore(
    ...
    startListeningForComponentMapEventForwarders(componentMap, listenerMiddleware);
...
function createSofterReducerFromComponentMap(componentMap: Record<string, ComponentDef>) {
    return createReducer(initialStateFromComponentMap(componentMap), (builder: any) => {
        Object.entries(componentMap).forEach(([path, def]) => {
            Object.entries(def.stateUpdaters ?? {}).forEach(([updaterName, updater]) => {
                builder.addCase(`${path}${updaterName}`, (state: any, action: any) => {
...
function startListeningForComponentMapEventForwarders(componentMap: Record<string, ComponentDef>,
...
function startListeningForComponentEventForwarders(
    ...
    component.eventForwarders?.map(createListenerOption(path)).forEach((listenerOption) => {
